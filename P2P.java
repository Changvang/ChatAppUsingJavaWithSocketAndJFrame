...
public class P2P extends JFrame implements ActionListener {
	...
	public P2P() throws UnknownHostException, IOException {
		panel = new JPanel();
            ...
            public void actionPerformed(ActionEvent e) {
               if(file != null) {
            	   ...
            	   try {
       				...
       				send_socket = new Socket(InetAddress.getLocalHost(), PARTNER_PORT);
		...
		received_socket = server.accept(); // using to received massage
		...
		while (true) {
			try {
				...
				if(isFound && string.split(":")[0].contentEquals("SendFile")) {
					...
					t.start();
					// khoi dong lai socket vi sau khi gui file no tu dong
				...
				System.out.println("Loi nhan message");
				ChatHistory.setText(ChatHistory.getText() + '\n'
						+ "Message sending fail:Network Error");
				try {
					Thread.sleep(3000);
